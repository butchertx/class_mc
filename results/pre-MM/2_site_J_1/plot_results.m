...
for j = 1:length(J)
	...
	for i = 1:6
		mag2_mat(i, :) = alpha1.*mag2((36*(j - 1) + (i - 1)*interval1 + 1):(36*(j - 1) +i*interval1));
		binder_mat1(i, :) = binders((36*(j - 1) +(i - 1)*interval1 + 1):(36*(j - 1) +i*interval1));
		binder_err_mat1(i, :) = binder_err((36*(j - 1) +(i - 1)*interval1 + 1):36*(j - 1) +(i*interval1));
		alpha_mat1(i, :) = alpha1;
	end
